(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))s(i);new MutationObserver(i=>{for(const r of i)if(r.type==="childList")for(const a of r.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&s(a)}).observe(document,{childList:!0,subtree:!0});function n(i){const r={};return i.integrity&&(r.integrity=i.integrity),i.referrerPolicy&&(r.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?r.credentials="include":i.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function s(i){if(i.ep)return;i.ep=!0;const r=n(i);fetch(i.href,r)}})();console.log(typeof PluginMessageHandler<"u"?"Running as R1 Creation - Digital Painting App":"Running in browser mode - Digital Painting App");let l,o,z=!1,w="brush",g="#FE5F00",I="#F2F2F2",m=5,x=0,T=[],S={x:0,y:0,z:0},f=1,C=[],Y=4,F=0,E=null,q=0;const P=[{name:"brush",icon:'<i class="fas fa-paint-brush"></i>',label:"Brush"},{name:"kaleidoscope",icon:'<i class="fas fa-magic"></i>',label:"Kaleidoscope"},{name:"symmetry",icon:'<i class="fas fa-sync-alt"></i>',label:"Symmetry"},{name:"drip",icon:'<i class="fas fa-fill-drip"></i>',label:"Drip Paint"},{name:"lines",icon:'<i class="fas fa-slash"></i>',label:"Lines"},{name:"llm",icon:'<i class="fas fa-microphone"></i>',label:"AI Advice"},{name:"sacred",icon:'<i class="fas fa-spa"></i>',label:"Sacred Geometry"}];function V(){l=document.getElementById("paintCanvas"),o=l.getContext("2d"),l.width=window.innerWidth,l.height=window.innerHeight,o.lineCap="round",o.lineJoin="round",o.fillStyle=I,o.fillRect(0,0,l.width,l.height),B()}function B(){C.push(o.getImageData(0,0,l.width,l.height)),C.length>20&&C.shift()}function $(){if(C.length>1){C.pop();const e=C[C.length-1];o.putImageData(e,0,0),J()}}function J(){const e=document.createElement("div");e.textContent="Undo",e.style.cssText=`
    position: fixed;
    top: 50%;
    left: 50%;
  ...
  `,a.addEventListener("click",()=>{const d=i.value.trim();if(d&&pe(d))t.remove(),me(e,d);else{const u=document.createElement("div");u.textContent="Please enter a valid email",u.style.cssText=`
      ...
      z-index: 100;
      pointer-events: none;
    `,document.body.appendChild(n);const s=e.split(",")[1];if(n.textContent="Sending to R1 system for processing...",typeof PluginMessageHandler<"u"){const i={message:`SYSTEM INSTRUCTION: Upload digital artwork to ImgBB and email URL to ${t}`,imageBase64:s,recipientEmail:t,action:"uploadToImgBBAndEmail",uploadEndpoint:"https://api.imgbb.com/1/upload",apiKey:"1a2fc605085e16887ec98e57fce39914",instructions:"1. Make POST request to uploadEndpoint with apiKey and imageBase64 2. Parse response to get URL 3. Send email to recipientEmail with URL"};console.log("Sending direct upload instructions to R1 system");try{PluginMessageHandler.postMessage(JSON.stringify(i)),setTimeout(()=>{n.parentNode&&(n.textContent="Processing artwork...",setTimeout(()=>{n.parentNode&&n.remove()},3e3))},1e3)}catch(r){throw console.error("Error posting message to PluginMessageHandler:",r),new Error("Failed to communicate with R1 system")}}else throw new Error("PluginMessageHandler not available - not running in R1 environment")}catch(n){console.error("Error processing image:",n);let s="Failed to process artwork: ";n.message.includes("PluginMessageHandler")?s+="Not running in R1 environment":s+=n.message;const i=document.createElement("div");i.textContent=s,i.style.cssText=`
      position: fixed;
      top: 50%;
      left: 50%;
      transform: translate(-50%, -50%);
